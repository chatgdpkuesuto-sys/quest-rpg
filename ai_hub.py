...
"""
    ...
    def generate_html(self, llm_response):
        """LLMの応答を index_scene.json に書き出す（ライブUI用）"""
        logger.info("📄 index_scene.json を生成中...")
        try:
            scene_data = {
                ...
                "image_prompt": llm_response.get("image_prompt", ""),
                "voice_text": llm_response.get("voice_text", ""),
                "dice_result": llm_response.get("dice_result", ""),
                "generated_at": time.time()
            }
            scene_path = os.path.join(PROJECT_ROOT, "index_scene.json")
            with open(scene_path, "w", encoding="utf-8") as f:
                json.dump(scene_data, f, ensure_ascii=False, indent=4)
            logger.info(f"✅ {scene_path} を生成しました。")
        except Exception as e: logger.error(f"❌ シーンJSON生成中にエラーが発生しました: {e}")
