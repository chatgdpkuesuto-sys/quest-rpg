...
# ② 【ルート分岐・事前生成設定】
# 合計10枚のルート画像をダッシュボード向けに事前生成する
SCENES = [
    {
        "prefix": "route_love", 
        "prompt": "Serena Siluria, gardevoir gijinka, 1girl, solo, gentle smile, blushing deeply, hands on chest, looking up affectionately, soft lighting, bedroom background, romantic atmosphere",
        "count": 4
    },
    {
        "prefix": "route_lust", 
        "prompt": "Serena Siluria, gardevoir gijinka, 1girl, solo, ahegao, rolling eyes, impossible extreme blush, drooling, messy hair, straddling, intense passion, dark lighting, sweaty texture",
        "count": 3
    },
     {
        "prefix": "route_special", 
        "prompt": "Serena Siluria, gardevoir gijinka, 1girl, solo, glowing eyes, trance state, mysterious aura, holding a magical artifact, dark magical forest background, dramatic lighting",
        "count": 3
    }
]
                    ...
                    if 'images' in node_output:
                        for image in node_output['images']:
                            ...
                            if output_path:
                                ...
                                if getattr(generate_image, "custom_filename", None):
                                    save_filename = generate_image.custom_filename
                                else:
...
if __name__ == "__main__":
    ...
    # 🌟 NEW: Live Server Sync Mode (分岐生成) 🌟
    # コマンドライン引数 "sync" などを渡されたらこちらを実行
    if len(sys.argv) > 1:
        print("🚀 Live Server Sync Mode 起動 (ルート分岐生成)")
        ...
        # 2. ルート分岐とプロンプトの動的生成
        base_prompt = "Serena Siluria, gardevoir gijinka, 1girl, solo, "
        
        if "慈愛" in action:
            prompt_arg = base_prompt + "gentle smile, blushing deeply, hands on chest, looking up affectionately, soft lighting, bedroom background, romantic atmosphere"
            print("  💕 慈愛・調教ルート を検知。")
        elif "淫靡" in action:
            prompt_arg = base_prompt + "ahegao, rolling eyes, impossible extreme blush, drooling, messy hair, straddling, intense passion, dark lighting, sweaty texture"
            print("  😈 淫靡・嗜虐ルート を検知。")
        elif "特殊" in action:
            prompt_arg = base_prompt + "glowing eyes, trance state, mysterious aura, holding a magical artifact, dark magical forest background, dramatic lighting"
            print("  👁️ 特殊・覚醒ルート を検知。")
        else:
            # フォールバック（引数やデフォルト）
            prompt_arg = base_prompt + "standing, full body, looking at viewer, standard pose"
        ...
        print("🚀 Auto Illustrator v2.0 (Pre-Generation Mode) 起動")
        ...
        for scene in SCENES:
            ...
            for i in range(1, scene["count"] + 1):
                 generate_image.custom_filename = f"{prefix}_{i}.png"
                 generate_image(scene["prompt"])
                 
        print("\n🎉 全ての事前生成が完了しました！ダッシュボードで即時切替が可能です！")