// ==============================================
// 00_Dashboard: OBS Overlay Engine
// ==============================================
...
function playHeartbeat(intensity) {
    ...
    // Volume envelope (two thumps per beat) "do-kun"
    gainNode.gain.setValueAtTime(0, audioCtx.currentTime);
    gainNode.gain.linearRampToValueAtTime(1.0, audioCtx.currentTime + 0.05); // thump 1
    gainNode.gain.exponentialRampToValueAtTime(0.1, audioCtx.currentTime + 0.15);
    gainNode.gain.linearRampToValueAtTime(0.8, audioCtx.currentTime + 0.2); // thump 2
...
// 発情度に応じた心音ループの開始
function startHeartbeatLoop(arousalLevel) {
    if (heartbeatInterval) clearInterval(heartbeatInterval);

    if (arousalLevel < 30) return; // 30未満は心音なし

    // 興奮度が高いほどBPMが早くなる (1000ms 〜 400ms)
...
function applyGameState() {
    const state = typeof GAME_STATE !== 'undefined' ? GAME_STATE : { arousal: 0, despair: 0 };
    ...
    if (state.current_bg) {
        bgImage.src = state.current_bg;
    }

    // Arousal Effects (Heartbeat BGM + Visual Pulse)
    if (state.arousal >= 80) {
    ...
    // Start audio loop based on arousal
    startHeartbeatLoop(state.arousal);
}

// === Live Server Auto-Reload Hook ===
// Since Live server reloads the entire page on file save, we want
// to try and auto-play the audio without clicking if the context was previously resumed.
// *Note: Some modern browsers strictly block this on reload without user interaction.
// If audio fails, the user will just click [CLICK TO IGNITION] again.
window.addEventListener('load', () => {
    // Attempt auto-start (might be blocked by browser)
