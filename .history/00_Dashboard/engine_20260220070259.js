...
async function pollGameState() {
    try {
        ...
        if (state.current_image) {
            // 初回ロード時、またはタイムスタンプが更新された場合に画像リロード
            if (!window.lastImageTimestamp || state.timestamp !== window.lastImageTimestamp) {
                // Live Serverのルートからの絶対パスに変換 (../outputs -> /outputs)
                const safePath = state.current_image.replace('../', '/');

                // キャッシュ回避のため ?t=タイムスタンプ を付与
                bgImage.src = safePath + "?t=" + (state.timestamp || new Date().getTime());
