...
async function pollGameState() {
    try {
        ...
        // URLの末尾に以前の ?t= が付いているため、パスだけ抜き出して比較するか、
        // status.json からの更新シグナル（または単純な変更）で強制更新する
        if (state.current_image) {
            const currentPath = new URL(bgImage.src, window.location.href).pathname;
            const targetPath = state.current_image.replace('../', '/');

            // パス自体が変わった場合、または status.json 側でタイムスタンプが更新された場合に画像リロード
            if (!bgImage.src.includes(state.current_image) || state.timestamp !== window.lastImageTimestamp) {
                bgImage.src = state.current_image + "?t=" + new Date().getTime();
