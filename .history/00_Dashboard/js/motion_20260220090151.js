...
window.updateVisuals = function () {
    ...
    // 4. 新たな3D傾き効果 (Pseudo-3D Parallax Tilt: 画像の円運動)
    // 🌟 イラスト自体が「フープ」や「転がるコイン」のように、四隅を順ぐりに曲げながら円を描いて動く
    const maxTilt = 10 + (avgGauge * 20); // 最大傾斜角 (10度〜30度)

    // XとYの傾きに位相をずらした波（sinとcos）を使うことで、イラスト自体がぐるぐると立体的に回る
    const rotX = Math.sin(window.time * 1.5) * maxTilt;
    const rotY = Math.sin(window.time * 1.5) * maxTilt;
