...
window.updateVisuals = function () {
    ...
    // 4. 新たな3D傾き効果 (Pseudo-3D Parallax Tilt: 上下特化)
    // 🌟 ユーザー要望により「左右ではなく上下の傾き」のみに特化させる
    // ゲージマックス時は大きく前後に頷く（仰け反る）ような動きになる
    const maxTilt = 10 + (avgGauge * 20); // 最大傾斜角 (10度〜30度)

    // 縦の傾き（X軸回転）のみを強くかける。波の周期も少し早めに。
    const rotX = Math.sin(window.time * 1.5) * maxTilt;
    const rotY = 0; // 横の傾き（Y軸回転）は完全にゼロにする
