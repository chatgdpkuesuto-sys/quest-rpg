...
window.updateVisuals = function () {
    ...
    // 4. 新たな3D傾き効果 (Pseudo-3D Parallax Tilt)
    // 🌟 「四隅を曲げて戻す」ような3D感を演出するため、円運動に連動して画像を傾ける
    // ゲージマックス時は傾きも最大になり、よりダイナミックに波打つような印象になる
    const maxTilt = 8 + (avgGauge * 15); // 最大傾斜角 (8度〜23度)
    const rotX = Math.sin(window.time * 0.9) * maxTilt; // 縦の傾き
    const rotY = Math.cos(window.time * 1.2) * maxTilt; // 横の傾き
