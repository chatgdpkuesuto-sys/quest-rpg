...
async function pollGameState() {
    try {
        ...
        // 🌟 NEW: Update Monologue/Narrative Text
        const monologueContainer = document.getElementById('monologue-container');
        if (state.current_monologue && window.lastMonologue !== state.current_monologue) {
            window.lastMonologue = state.current_monologue;

            // 既存のモノローグをクリア
            monologueContainer.innerHTML = '';

            const monoChunks = state.current_monologue.split(/<br>/g).filter(s => s.trim().length > 0);

            monoChunks.forEach((text, index) => {
                setTimeout(() => {
                    const textDiv = document.createElement('div');
                    textDiv.className = 'monologue-text';
                    textDiv.innerHTML = text; // HTML要素を許容。強調表示などに使うため

                    monologueContainer.appendChild(textDiv);
