...
class GameFrontEnd:
    ...
    def _apply_state(self, state: dict):
        ...
        if voice_path and Path(voice_path).exists():
            if voice_path != self.last_played_voice_path:
                self.renderer.play_audio(voice_path)
                self.last_played_voice_path = voice_path
        elif voice_text:
            # 毎回一意のテキストなら別処理だが、簡易的に対応
            def _gen_audio():
                try:
                    path = self.voicevox.synthesize_dialogue(voice_text[:200])
                    if path and str(path) != self.last_played_voice_path:
                        self.renderer.play_audio(str(path))
                        self.last_played_voice_path = str(path)
                except Exception as e:
