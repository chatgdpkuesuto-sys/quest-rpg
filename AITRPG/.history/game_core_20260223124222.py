...
from d20_engine import Character, D20Engine, SOUL_CARDS, JOB_CARDS, SKILL_DB
...
class GameCore:
    ...
    def __init__(self):
        ...
        self._building_party: List[dict] = []  # {soul, job, skills}
        self._current_member_idx: int = 0
        self._current_skill_picks: List[str] = []
    ...
    def _start_skill_selection(self, member_num: int, pick_num: int):
        """仲間N人目のスキル選択画面（2つ選ぶ）"""
        self.creation_phase = f"SKILL_{member_num}_{pick_num}"
        self._current_member_idx = member_num
        ...
        # すでに選んだスキルを除外
        already = info.get("skills", [])
        available = [s for s in all_skills if s not in already]
        picks = random.sample(available, min(3, len(available)))
        self._available_skills_list = picks

        msg = f"【{info['soul']}({job})のスキル選択 {pick_num}/2】\n\n習得するスキルを選んでください。"
    ...
    def _handle_creation_choice(self, choice_index: int) -> bool:
        ...
        if phase.startswith("MEMBER_"):
            ...
            self._building_party.append({"soul": chosen_soul, "job": "", "skills": []})
            ...
            # スキル選択（1つ目）
            self._start_skill_selection(num, 1)

        elif phase.startswith("SKILL_"):
            parts = phase.split("_")
            num = int(parts[1])
            pick_num = int(parts[2])
            chosen_skill = self._available_skills_list[choice_index]
            self._building_party[num - 1]["skills"].append(chosen_skill)
            print(f"[GameCore] スキル習得: {chosen_skill}")

            if pick_num < 2:
                # スキル2つ目
                self._start_skill_selection(num, 2)
            elif num < 3:
                # 次のメンバー選択
    ...
    def _finalize_party(self):
        ...
        for info in self._building_party:
            pc = Character(info["soul"], info["job"], info["skills"])
