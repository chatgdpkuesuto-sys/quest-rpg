...
class GameCore:
    ...
    def _start_member_selection(self, member_num: int):
        ...
        def _fetch_candidates():
            ...
            self.current_soul_picks = candidates
            ...
            self.renderer.set_scene(scene_text=msg, choices=choices_text)
    ...
    def _finalize_party(self):
        ...
        def _gen_chara_cards():
            for m in state.party:
                try:
                    # ランダムキャラの場合は、originやpersonalityを含めてComfyUIに渡すように依頼する
                    prompt_str = f"{m.soul_card}, {m.origin}, {m.personality}"
                    path = self.comfyui.generate_party_chara(prompt_str, m.job_card)
                    if path:
