...
class GameCore:
    ...
    def _setup_character(self) -> str:
        """コンソールでキャラ作成を行う"""
        print("\n=== キャラクター・メイキング ===")
        souls = {"1": "孫悟空", "2": "フリーレン", "3": "ルフィ", "4": "キリト", "5": "アーニャ"}
        jobs = {"1": "戦士", "2": "魔法使い", "3": "盗賊", "4": "僧侶"}
        
        print("\n[魂カードを選択]")
        for k, v in souls.items(): print(f"  {k}: {v}")
        s_idx = input("> 番号を入力 (デフォルト: 1): ").strip()
        soul = souls.get(s_idx, "孫悟空")
        
        print("\n[ジョブカードを選択]")
        for k, v in jobs.items(): print(f"  {k}: {v}")
        j_idx = input("> 番号を入力 (デフォルト: 1): ").strip()
        job = jobs.get(j_idx, "戦士")
        
        pc = Character(soul, job)
        self.state_mgr.state.character = pc
        
        print(f"\n=> 【{soul}】×【{job}】の冒険者が誕生しました！")
        return f"プレイヤーは『{soul}の魂』を持ち『{job}の職業』に就いた冒険者として、暗黒大陸ノクターナルで目覚めるところから始めてくれ。"

    def run(self):
        """メインゲームループ。"""
        try:
            # キャラ作成
            intro_msg = self._setup_character()
