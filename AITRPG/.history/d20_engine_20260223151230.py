...
class Character:
    ...
    def __init__(self, soul_card: str, job_card: str,
        ...
        self.personality: str = str(personality or soul.get("personality", "普通"))
        ob = soul.get("obedience", 50)
        self.obedience: int = int(ob) if isinstance(ob, (int, float, str)) and str(ob).isdigit() else 50
        self.origin: str = str(origin or soul.get("origin", "不明"))

        # --- キャラスキル（魂ごとに固定1つ、構造化） ---
        chara = soul.get("chara_skill", {})
        if not isinstance(chara, dict):
            chara = {}
        self.chara_skill_name: str = str(chara.get("name", ""))
        self.chara_skill_desc: str = str(chara.get("desc", ""))
        self.chara_skill_trigger: str = str(chara.get("trigger", ""))
        
        eff = chara.get("effect", {})
        self.chara_skill_effect: dict = eff if isinstance(eff, dict) else {}
        
        self.chara_skill_limit: int = int(chara.get("limit", 0))
        self.chara_skill_recharge: str = str(chara.get("recharge", ""))
        self.chara_skill_uses: int = 0  # 使用済み回数

        self.innate_skills: List[dict] = []  # 構造化スキルで管理
        w = soul.get("weaknesses", [])
        self.weaknesses: List[str] = w if isinstance(w, list) else []
        ...
        # --- ステータス補正適用 ---
        sm = soul.get("stat_mods", {})
        if isinstance(sm, dict):
            for stat, mod in sm.items():
