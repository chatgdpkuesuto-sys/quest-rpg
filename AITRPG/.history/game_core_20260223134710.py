...
class GameCore:
    ...
    def _finalize_party(self):
        ...
        for soul_name in self._building_party:
            ...
            party_lines.append(
                ...
                f"    ジョブスキル: {job_skill} / 汎用: {uni_skill}"
            )
    ...
    def _enter_combat(self, enemies: List[Enemy]):
        ...
        for i, m in enumerate(state.party):
            if m.is_alive and not m.is_downed:
                roll = random.randint(1, 20) + m.get_modifier("DEX")
                initiative_results.append(("pc", i, roll, m.get_display_name()))
    ...
    def _process_pc_combat_action(self, member: Character, choice_index: int):
        ...
        instruction = (
            f"絶対厳守: 以下の判定結果に従って描写すること。\n{dice_report}\n"
            f"戦闘中: ラウンド{state.combat_round}"
        )
        enemy_status = " / ".join(
        ...
        self._generate_scene(user_msg, instruction, combat_mode=True)
    ...
    def _process_enemy_turn(self, enemy: Enemy):
        ...
        instruction = f"絶対厳守: 以下の判定結果に従って描写すること。\n{dice_report}"
    ...
    def _process_death_save(self, member: Character):
        ...
        if result.get("nat20"):
            instruction = f"絶対厳守: {member.soul_card}が奇跡的にHP1で意識を取り戻した！\n{dice_report}"
        elif result.get("died"):
            instruction = f"絶対厳守: {member.soul_card}が息を引き取った...\n{dice_report}"
        elif result.get("stabilized"):
            instruction = f"絶対厳守: {member.soul_card}の容態が安定した。\n{dice_report}"
        else:
            instruction = f"絶対厳守: {member.soul_card}の死亡セーヴ結果:\n{dice_report}"
