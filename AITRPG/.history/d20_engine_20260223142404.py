...
SOUL_CARDS: Dict[str, dict] = {
    "孫悟空": {
        ...
        "personality": "熱血",
        "obedience": 30,
        "chara_skill": {"name": "激怒", "desc": "HP半分以下で近接ダメ+2、STR判定有利"},
        "innate_skills": [
    ...
    "フリーレン": {
        ...
        "personality": "冷静",
        "obedience": 60,
        "chara_skill": {"name": "ゾルトラーク", "desc": "武器なしで1d10の魔法遠距攻撃"},
        "innate_skills": [
            {"name": "ゾルトラーク", "desc": "武器なしで1d10の魔法遠距攻撃"},
        ],
    ...
    "ルフィ": {
        ...
        "personality": "天然",
        "obedience": 20,
        "chara_skill": {"name": "ゴムゴムの体", "desc": "打撃ダメージ半減、電撃無効"},
        "innate_skills": [
    ...
    "キリト": {
        ...
        "personality": "孤高",
        "obedience": 50,
        "chara_skill": {"name": "二刀流", "desc": "近接攻撃時、ボーナスアクションで追加攻撃"},
        "innate_skills": [
    ...
    "アーニャ": {
        ...
        "personality": "マイペース",
        "obedience": 25,
        "chara_skill": {"name": "心を読む", "desc": "NPC交渉・嘘看破に常に有利"},
        "innate_skills": [
...
class Character:
    ...
    def from_dict(d: dict) -> "Character":
        ...
        c.chara_skill_name = d.get("chara_skill_name", "")
        c.chara_skill_desc = d.get("chara_skill_desc", "")
...
if __name__ == "__main__":
    print("=" * 60)
    print("  D20コアエンジン v2 テスト（50スキル版 + SkillEffect）")
    print("=" * 60)
    ...
    for m in party:
        ...
        print(f"    ★キャラ: {m.chara_skill_name}（{m.chara_skill_desc}）")
    ...
    print(f"\n--- SkillEffect解釈テスト ---")
    test_skills = [
        ("ファイアボール", "3d6火炎（DEX半減）", "アクション"),
        ("キュア", "1d8+WIS回復", "アクション"),
        ("パリィ", "AC+1d6（1攻撃）", "リアクション"),
        ("マジックミサイル", "必中1d4+1×3", "アクション"),
        ("渾身の一撃", "命中-5、命中時+10ダメ", "パッシブ"),
        ("急所攻撃", "1T1回+1d6（Lvで増加）", "パッシブ"),
        ("スリープ", "1体行動不能（WISセーヴ、1T）", "アクション"),
    ]
    for name, desc, typ in test_skills:
        eff = interpret_skill(name, desc, typ)
        parts = [f"  {name}: type={eff['action_type']}"]
        if eff["damage"]: parts.append(f"dmg={eff['damage']}")
        if eff["heal"]: parts.append(f"heal={eff['heal']}")
        if eff["ac_mod"]: parts.append(f"ac={eff['ac_mod']:+d}")
        if eff["to_hit_mod"]: parts.append(f"hit={eff['to_hit_mod']:+d}")
        if eff["auto_hit"]: parts.append("必中")
        if eff["save"]: parts.append(f"save={eff['save']}")
        if eff["condition"]: parts.append(f"cond={eff['condition']}")
        print(" | ".join(parts))
