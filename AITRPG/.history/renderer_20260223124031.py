...
class Renderer:
    ...
    def _draw_party_status(self):
        """画面左上にパーティー3人分のステータスを描画"""
        if not self.party_info:
            return
        ...
        member_h = 48
        padding = 8
        total_h = padding * 2 + member_h * len(self.party_info)
        x, y = margin, margin

        # 背景
        bg_surface = pygame.Surface((box_w, total_h), pygame.SRCALPHA)
        bg_surface.fill((15, 10, 8, 210))
        self.screen.blit(bg_surface, (x, y))
        pygame.draw.rect(self.screen, self.COLOR_SCENE_BORDER, (x, y, box_w, total_h), 2)

        text_y = y + padding
        for m in self.party_info:
            name = m.get("name", "???")
            hp = m.get("hp", 0)
            ...
            # 名前行
            name_color = self.COLOR_HIGHLIGHT if alive else self.COLOR_DIM
            name_surf = self.font_small.render(name, True, name_color)
            self.screen.blit(name_surf, (x + 10, text_y))
            ...
            self.screen.blit(hp_surf, (bar_x + bar_w - hp_surf.get_width() - 2, bar_y - 2))
