...
class LMClient:
    ...
    def generate_candidates(self, exclude_names: list[str] = None) -> list[dict]:
        ...
        sys_prompt = (
            ...
            "出力は必ず以下のJSON配列形式のみとし、他のテキストは一切出力しないでください。\n"
            "※重要※ 下記のJSON内に書かれている「(生成した名前)」などの例文や注釈文字は絶対にそのまま出力せず、あなたが選んだ具体的なキャラクターの情報だけを出力してください。\n"
            "[\n"
            "  {\n"
            "    \"name\": \"(生成したキャラクターの日本語名)\",\n"
            "    \"name_en\": \"(そのキャラクターの英語名、例: Asuna)\",\n"
            "    \"origin\": \"(そのキャラクターの原作の日本語名)\",\n"
            "    \"origin_en\": \"(その原作の英語名、例: Sword Art Online)\",\n"
            "    \"personality\": \"(性格の日本語説明)\",\n"
            "    \"desc\": \"(外見やスキルの日本語説明)\"\n"
            "  },\n"
            "  ...(これを3人分出力)\n"
            "]"
