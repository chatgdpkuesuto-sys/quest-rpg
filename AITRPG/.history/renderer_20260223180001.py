...
class Renderer:
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        if self.fullscreen:
            ...
            self.screen_w = max(800, 1280)
            self.screen_h = max(600, 720)
        ...
        # 生画像保持用（リサイズ対応）
        self.raw_bg: Optional[pygame.Surface] = None
        self.raw_chara: Optional[pygame.Surface] = None
        self.raw_prop: Optional[pygame.Surface] = None
    ...
    def set_scene(
        ...
        if image_path and Path(image_path).exists():
            try:
                self.raw_bg = pygame.image.load(str(image_path)).convert()
                self.current_image = self._fit_image(self.raw_bg)
                self.fading_in = True
                self.fade_alpha = 0
        ...
        if chara_path and Path(chara_path).exists():
            try:
                self.raw_chara = pygame.image.load(str(chara_path)).convert_alpha()
                self.current_chara = self._fit_chara_image(self.raw_chara)
        ...
        if prop_path and Path(prop_path).exists():
            try:
                self.raw_prop = pygame.image.load(str(prop_path)).convert_alpha()
                self.current_prop = self._fit_prop_image(self.raw_prop)
            except Exception as e:
    ...
    def _fit_card_image(self, surface: pygame.Surface, target_w: int, target_h: int) -> pygame.Surface:
        ...
        # 顔が見えるように、縦方向のクロップ位置を「中央」ではなく「上から15%程度の位置」に調整
        x = max(0, (new_w - target_w) // 2)
        y = max(0, int((new_h - target_h) * 0.15))
    ...
    def _handle_events(self) -> Optional[Union[int, str]]:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                self.scroll_y = max(0.0, min(self.scroll_y, self.max_scroll_y))
    ...
    def _handle_resize(self, w: int, h: int):
        """ウィンドウサイズ変更時の再計算"""
        self.screen_w = max(800, w)
        self.screen_h = max(600, h)
        self.screen = pygame.display.set_mode((self.screen_w, self.screen_h), pygame.RESIZABLE)
    ...
    def _draw_text_overlay(self):
        ...
        margin_y = 25
        # スクロールバー用に少し右を空ける
        max_width = self.screen_w - margin_x * 2 - 20

        # クリッピング領域（テキストが枠外にはみ出ないようにする）
        clip_rect = pygame.Rect(margin_x, overlay_y + margin_y, max_width, overlay_h - margin_y * 2)
        self.screen.set_clip(clip_rect)

        total_text_h = self._draw_wrapped_text(
            visible_text,
            margin_x,
            overlay_y + margin_y - int(self.scroll_y),
            max_width,
            self.font,
            self.COLOR_TEXT,
        )
        ...
        # スクロールバー関連の計算
        visible_h = overlay_h - margin_y * 2
        
        if total_text_h > visible_h:
            self.max_scroll_y = float(total_text_h - visible_h)
            # テキスト出力中なら、自動で下まで追従スクロールさせる
            if not self.text_complete:
                self.scroll_y = self.max_scroll_y
                
            # スクロールバーの描画
            bar_w = 6
            bar_x = self.screen_w - margin_x + 10
            bar_y = overlay_y + margin_y
            bar_h = visible_h
            
            # スクロールバー背景
            pygame.draw.rect(self.screen, (50, 40, 30), (bar_x, bar_y, bar_w, bar_h))
            
            # スクロールバーつまみ
            knob_h = max(20, int((visible_h / total_text_h) * visible_h))
            knob_y = bar_y + int((self.scroll_y / self.max_scroll_y) * (bar_h - knob_h))
            pygame.draw.rect(self.screen, self.COLOR_HIGHLIGHT, (bar_x, knob_y, bar_w, knob_h))
        else:
            self.max_scroll_y = 0.0
            self.scroll_y = 0.0
    ...
    def _draw_party_status(self):
        ...
        # 画面サイズに応じたカードの動的リサイズ設定
        card_h = max(180, min(310, int(self.screen_h * 0.4)))
        card_w = int(card_h * (210 / 310))
        spacing = int(card_w * 0.15)
        margin_right = max(20, int(self.screen_w * 0.03))
        margin_bottom = max(70, int(self.screen_h * 0.1))  # 入力バーの少し上
        ...
        for i, m in enumerate(party_info):
            ...
            # カード画像の描画
            if image_path and Path(image_path).exists():
                cache_key = f"{image_path}_{card_w}_{card_h}"
                if cache_key not in self.party_images_cache:
                    try:
                        raw_img = pygame.image.load(str(image_path)).convert_alpha()
                        self.party_images_cache[cache_key] = self._fit_card_image(raw_img, card_w, card_h)
                    except Exception as e:
                        print(f"[Renderer] カード画像読み込みエラー: {e}")
                        # エラー時はダミーを入れる
                        dummy = pygame.Surface((card_w, card_h), pygame.SRCALPHA)
                        dummy.fill((25, 30, 35, 230))
                        self.party_images_cache[cache_key] = dummy
                
                card_img = self.party_images_cache[cache_key]
