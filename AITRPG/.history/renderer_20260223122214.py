...
class Renderer:
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        self.character_info: Optional[dict] = None  # ダイス/ステータス管理用
    ...
    def _draw_character_status(self):
        """画面左上にステータスボックスを描画する"""
        ...
        bg_surface.fill((30, 20, 15, 200)) # 暗めの半透明
        self.screen.blit(bg_surface, (x, y))
        pygame.draw.rect(self.screen, self.COLOR_SCENE_BORDER, (x, y, box_w, box_h), 2)

        # テキスト
        c = self.character_info
        lines = [
            f"CLASS: {c.get('soul', '-')} × {c.get('job', '-')}",
            f"HP: {c.get('hp', 0)}/{c.get('max_hp', 0)}  |  AC: {c.get('ac', 10)}",
            f"STR:{c.get('STR', 0):+d}  DEX:{c.get('DEX', 0):+d}  CON:{c.get('CON', 0):+d}",
            f"INT:{c.get('INT', 0):+d}  WIS:{c.get('WIS', 0):+d}  CHA:{c.get('CHA', 0):+d}",
            f"SKILL: {c.get('skills', '')}"
        ]

        text_y = y + 15
        for line in lines:
            surf = self.font_small.render(line, True, self.COLOR_HIGHLIGHT)
            self.screen.blit(surf, (x + 15, text_y))
            text_y += 28
