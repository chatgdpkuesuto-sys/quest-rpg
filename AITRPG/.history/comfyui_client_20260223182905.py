...
class ComfyUIClient:
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        self.charapose_workflow_file = r"c:\Users\kuesu\OneDrive\Desktop\AITRPG\CHARAPOSE.json"
        ...
        self.charapose_workflow_template = self._load_workflow(self.charapose_workflow_file)
    ...
    def generate_party_chara(self, soul_name: str, job_name: str = "") -> Optional[Path]:
        ...
        try:
            ...
            if img_info is not None:
                ...
                # 魂名（プロンプト）が長すぎるとファイルパス長制限に引っかかるため、短縮＋タイムスタンプで保存
                save_dir = Path(__file__).parent / "assets" / "characters"
                save_dir.mkdir(parents=True, exist_ok=True)
                timestamp = int(time.time() * 1000)
                safe_name = "".join(c for c in soul_name if c.isalnum() or c in " _-")[:20].strip().replace(" ", "_")
                if not safe_name:
                    safe_name = "chara"
                save_path = save_dir / f"card_{safe_name}_{timestamp}.png"
                with open(save_path, "wb") as f:
    ...
    # --- 8種キャラクターポーズ差分生成メソッド (CHARAPOSE.json) ---
    def generate_chara_poses(self, chara_name: str, base_prompt: str) -> dict[str, Path]:
        """
        指定されたキャラクターの8種類のポーズ差分を生成し、保存パスの辞書を返す。
        """
        if not self.charapose_workflow_template:
            print(f"[ComfyUI] CHARAPOSE.json がないためポーズ差分生成スキップ。")
            return {}

        poses = {
            "smile": "standing, looking at viewer, gentle smile",
            "serious": "standing, looking at viewer, serious",
            "surprised": "standing, looking at viewer, surprised",
            "sad": "standing, looking at viewer, sad, looking down",
            "blush": "standing, looking at viewer, blush",
            "action": "action pose, fighting stance, casting magic",
            "back": "from behind, back, looking away",
            "lookback": "from behind, looking back at viewer"
        }

        generated_paths = {}
        
        # 保存先ディレクトリ（キャラごとにまとめる）
        safe_name = "".join(c for c in chara_name if c.isalnum() or c in " _-").strip().replace(" ", "_")
        if not safe_name: safe_name = "chara"
        save_dir = Path(__file__).parent / "assets" / "characters" / safe_name
        save_dir.mkdir(parents=True, exist_ok=True)

        for pose_key, pose_desc in poses.items():
            positive = f"masterpiece, best quality, {base_prompt}, {pose_desc}"
            # 透過クロマキー指定があるなら追加する（ワークフロー側でRMBGで抜いていても背景色指定があったほうが安定しやすい）
            # positive += ", simple background, green background, chroma key"
            
            workflow = json.loads(json.dumps(self.charapose_workflow_template))
            
            # ノード23(テキスト), ノード4(ネガティブ)
            if "23" in workflow and "inputs" in workflow["23"]:
                workflow["23"]["inputs"]["text"] = positive
            ...
            print(f"[ComfyUI] 差分生成開始: {chara_name} - {pose_key}...")
            try:
                resp = requests.post(f"{self.base_url}/prompt", json=prompt_payload, timeout=10)
                resp.raise_for_status()
                prompt_id = resp.json().get("prompt_id")
                if not prompt_id:
                    print(f"[{pose_key}] プロンプトID取得失敗")
                    continue
                ...
                if img_info is not None:
                    params = urlencode({"filename": img_info["filename"], "subfolder": img_info.get("subfolder", ""), "type": img_info.get("type", "output")})
                    dl_resp = requests.get(f"{self.base_url}/view?{params}", timeout=10)
                    dl_resp.raise_for_status()

                    save_path = save_dir / f"pose_{pose_key}.png"
                    with open(save_path, "wb") as f:
                        f.write(dl_resp.content)
                    print(f"[ComfyUI] 生成完了: {save_path.name}")
                    generated_paths[pose_key] = save_path
                else:
                    print(f"[ComfyUI] {pose_key} タイムアウト")
                    
            except Exception as e:
                print(f"[ComfyUI] {pose_key} 生成失敗: {e}")

        return generated_paths

    # --- キャラクター単体生成メソッド（旧バージョン） ---
