...
class GameCore:
    ...
    def __init__(self):
        ...
        self.state = self.state_mgr.state # Added this line
        ...
        self.pending_scene_update: Optional[dict] = None # Added this line

        # キャラ作成
        self.creation_phase = "DONE"
        self.remaining_souls: List[str] = []
        self.current_soul_picks: List[dict] = []
        self._building_party: List[dict] = []  # 選んだキャラ情報のリスト
        self.seen_candidates: List[str] = []  # 一度生成したキャラ（重複防止用）
    ...
    def _start_member_selection(self, member_num: int):
        ...
        self.renderer.set_scene(scene_text=f"【パーティー編成（{member_num}/3）】\n\n未知の魂を召喚中...", choices=[])
        
        def _fetch_candidates():
            # candidates取得
            self.pending_scene_update = {
                "action": "update_text",
                "scene_text": f"【パーティー編成（{member_num}/3）】\n\n候補の姿を具現化中（画像生成）...\nしばらくお待ちください。"
            }
            
            candidates = self.lm.generate_candidates(exclude_names=self.seen_candidates)
            # candidates: [{"name": "...", "origin": "...", "personality": "...", "desc": "..."}, ...]
            self.current_soul_picks = candidates # Moved this line up
            ...
            for i, cand in enumerate(candidates):
                name = cand.get("name", "名無し")
                if name != "名無し" and name not in self.seen_candidates:
                    self.seen_candidates.append(name)
                    
                origin = cand.get("origin", "不明")
                personality = cand.get("personality", "普通")
                desc = cand.get("desc", "")
                
                # 事前にランダムなジョブを割り当てて画像を生成する
                cand_job = random.choice(jobs)
                cand["temp_job"] = cand_job
                
                name_en = cand.get("name_en", "")
                origin_en = cand.get("origin_en", "")
                
                if name_en or origin_en:
                    prompt_str = f"{name_en}, {origin_en}"
                else:
                    prompt_str = f"{name}, {origin}, {personality}"
                    
                try:
                    path = self.comfyui.generate_party_chara(prompt_str, cand_job)
                    choice_chara_paths.append(str(path) if path else None)
                except Exception as e:
                    print(f"[GameCore] 候補キャラ画像生成エラー({name}): {e}")
                    choice_chara_paths.append(None)
                ...
                choices_text.append(name)
            
            # メイン処理へ更新要求
            self.pending_scene_update = {
                "action": "finalize_fetch",
                "scene_text": msg,
                "choices": choices_text,
                "choice_chara_paths": choice_chara_paths
            }

        # 別スレッドでガチャ通信を実行（ブロックしないように）
        threading.Thread(target=_fetch_candidates, daemon=True).start()

    def _handle_creation_choice(self, choice_index: int) -> bool:
        """キャラ作成フェーズ処理"""
        if self.creation_phase == "DONE" or not hasattr(self, "current_soul_picks"):
            return False

        phase = self.creation_phase
        if phase.startswith("MEMBER_"):
            num = int(phase.split("_")[1])
            if choice_index >= len(self.current_soul_picks):
                return True
                
            chosen_data = self.current_soul_picks[choice_index]
            self._building_party.append(chosen_data)
            print(f"[GameCore] 仲間{num}: {chosen_data.get('name')} を選択")
    ...
    def _finalize_party(self):
        ...
        party_lines = []
        for cand in self._building_party:
            soul_name = cand.get("name", "ナナシ")
            origin = cand.get("origin", "不明")
            personality = cand.get("personality", "普通")
            
            # 候補事前生成時に割り当てたジョブを利用（画像と一致させるため）
            job = cand.get("temp_job", random.choice(jobs))
            job_skill = random.choice(list(SKILL_DB.get(job, {}).keys()))
            uni_skill = random.choice(list(UNIVERSAL_SKILLS.keys()))
            
            # Character生成時にoriginとpersonalityを渡す
            pc = Character(soul_name, job, job_skill=job_skill, universal_skill=uni_skill,
                           personality=personality, origin=origin)
    ...
    def _process_pc_combat_action(self, member: Character, choice_index: int):
        ...
        if choice_index == 0:
            ...
            # ジョブスキル
            skill_target = None
            # ダメージがないスキル(回復やバフ)なら味方をランダム選択
            eff = self.d20.use_skill(member, member.job_skill, target=None).get("effect", {})
            if eff.get("heal") or eff.get("target") in ("ALLY", "SELF"):
                skill_target = random.choice([m for m in state.party if m.is_alive])
            
            skill_result = self.d20.use_skill(member, member.job_skill, target=skill_target)
            ...
            # 汎用スキル
            skill_target = None
            eff = self.d20.use_skill(member, member.universal_skill, target=None).get("effect", {})
            if eff.get("heal") or eff.get("target") in ("ALLY", "SELF"):
                skill_target = random.choice([m for m in state.party if m.is_alive])
                
            skill_result = self.d20.use_skill(member, member.universal_skill, target=skill_target)
    ...
    def run(self):
        try:
            ...
            while self.running:
                # 保留中のUI更新があれば適用する（スレッドセーフ対策）
                if self.pending_scene_update:
                    update = self.pending_scene_update
                    self.pending_scene_update = None
                    if update["action"] == "update_text":
                        self.renderer.set_scene(scene_text=update["scene_text"], choices=[])
                    elif update["action"] == "finalize_fetch":
                        self.renderer.set_loading(False)
                        self.renderer.set_scene(
                            scene_text=update["scene_text"],
                            choices=update["choices"],
                            choice_chara_paths=update["choice_chara_paths"]
                        )
                        self.state_mgr.state.last_choices = update["choices"]
                        self.awaiting_choice = True
